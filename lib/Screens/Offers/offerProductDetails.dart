...
import 'package:ezhyper/Model/FavouriteModel/favourite_list_model.dart' as favourite_model;
...
class OfferProductDetails extends StatefulWidget {
  ...
  OfferProductDetails({this.offer_product,});
  @override
...
class _OfferProductDetailsState extends State<OfferProductDetails> {
              ...
              Stack(
                children: [
                  MyProductSlider(
                    ...
                    border_radius: 0.0,
                    indicator: false,
                    slider_height: StaticData.get_height(context)/2,
                    motion: true,
                  ),
                  Padding(
                        ...
                        Container(padding: EdgeInsets.all(height*.01),
                          ...
                          child: InkWell(
                            child:  translator.currentLanguage == 'ar' ? Image.asset(
                              ...
                              height: height * .03,
                            ) : Image.asset("assets/images/arrow_left.png",height: height*.015,),
                            onTap: (){
                          ...
                          child: Row(mainAxisAlignment: MainAxisAlignment.end,
                            children: [
                              InkWell(
                                ...
                                child:  Container(padding: EdgeInsets.all(height*.01),
                              ...
                              Container(padding: EdgeInsets.only(bottom:height*.01),
                                height: height*.05,width: width*.2,
                                decoration:BoxDecoration(shape: BoxShape.circle,
                                    color: whiteColor),
                                child: favourite_status ?  IconButton(
                                  icon: Icon(
                                    Icons.favorite,
                                    size: 20,
                                    color: redColor,
                    ...
                    whiteColor,borderRadius: BorderRadius.circular(height*.05)),child: Column(children: [
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            MyText(text: "${widget.offer_product.name} ", size: height*.02),
                          ],),
                      ),
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.start,
                        children: [
                          MyText(text: "${widget.offer_product.priceAfterDiscount} ${translator.translate("SAR")} ", size: height*.02),
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: [
                                RatingBar.readOnly(
                                  initialRating: widget.offer_product.totalRate.toDouble(),
                                  maxRating: 5,
                                  isHalfAllowed: true,
                                  ...
                                  filledColor: (widget.offer_product.totalRate.toDouble() >= 1)
                                ...
                                MyText(
                                  text: "(${widget.offer_product.countRates})",
                                  size: height * .015,
                                  color: greyColor,
                                )
                              ],
                    ...
                    Row(
                      ...
                      children: [
                        Container(
                          ...
                          child: Wrap(
                            children: [
                              MyText(text: "${widget.offer_product.description}",
                                size: height*.018,align: TextAlign.left,)
                            ],
                            ...
                            child: Row(
                              children: <Widget>[
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      if (qty <= 1) {
                                        errorDialog(
                                          context: context,
                                          text: "لقد نفذت الكمية من هذا المنتج",
                                        );
                                      } else {
                                        setState(() {
                                          qty--;
                                ...
                                MaterialButton(
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      print("prod_main_quantity : ${product_quantity}");
                                      if (qty ==
                                          product_quantity) {
                                        errorDialog(
                                          context: context,
                                          text:  "لا يمكنك تخطى الكمية المتاحة",
                                        );
                                      } else {
                                        setState(() {
                                          qty++;
                                ...
                                CustomSubmitAndSaveButton(buttonText: translator.translate("ADD TO CART"),
                                  btn_width: width * .60,
                                  onPressButton: (){
                                    print("cart 1");
                                    if(_cart_status ==0){
    ...
    return InkWell(splashColor:whiteColor,
      ...
      child: Container(width: width*.4,
        child: Column(children: [
          MyText(text: 'Product Details ' ,color:  product_status? greyColor: greenColor,size: height*.02,weight: FontWeight.bold,),
          Container(width: width*.4,color:  product_status? whiteColor: greenColor,height: height*.002,)
        ],),),
    );
  }
  Widget productReviewButton(){
    ...
    return InkWell(splashColor: whiteColor,
      ...
      child: Container(width: width*.4,
        child: Column(children: [
          MyText(text: 'Product Review' ,color: product_status? greenColor: greyColor ,size: height*.02,weight: FontWeight.bold,),
          Container(width: width*.4,color: product_status? greenColor: whiteColor,height: height*.002,)
        ],),),
    );
  }
  ...
  Widget product_details(){
    return  ListView(
      ...
      shrinkWrap: true,
        children: [
          SizedBox(height: StaticData.get_height(context)*.03,),
          Row(mainAxisAlignment: MainAxisAlignment.center,children: [
            Container(height: StaticData.get_height(context)*.08,
                padding: EdgeInsets.only(right: StaticData.get_width(context)*.05,left: StaticData.get_width(context)*.05),
                child: Row(children: [
                  MyText(text: "product number", size: StaticData.get_height(context)*.02),
                  MyText(text: "${widget.offer_product.code??translator.translate("not found") }", size: StaticData.get_height(context)*.02),

                ],mainAxisAlignment: MainAxisAlignment.spaceBetween,),
                width: StaticData.get_width(context),color: whiteColor)
          ],),
          SizedBox(height: StaticData.get_height(context)*.03,),
          Row(mainAxisAlignment: MainAxisAlignment.center,children: [
            Container(height: StaticData.get_height(context)*.1,
              padding: EdgeInsets.only(right: StaticData.get_width(context)*.05,left: StaticData.get_width(context)*.05),
              child: Row(children: [
                MyText(text: "product Size", size: StaticData.get_height(context)*.02),
                MyText(text: "${widget.offer_product.size??translator.translate("not found") }", size: StaticData.get_height(context)*.02),

              ],mainAxisAlignment: MainAxisAlignment.spaceBetween,),
              width: StaticData.get_width(context),color: backgroundColor,)
          ],),
          Row(mainAxisAlignment: MainAxisAlignment.center,children: [
            Container(height: StaticData.get_height(context)*.08,
                padding: EdgeInsets.only(right: StaticData.get_width(context)*.05,left: StaticData.get_width(context)*.05),
                child: Row(children: [
                  MyText(text: "Width", size: StaticData.get_height(context)*.02),
                  MyText(text:widget.offer_product.unit==null? translator.translate("not found") :  "${widget.offer_product.unit} ${translator.translate("Cm")}", size: StaticData.get_height(context)*.02),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center,children: [
            Container(height: StaticData.get_height(context)*.1,
              padding: EdgeInsets.only(right: StaticData.get_width(context)*.05,left: StaticData.get_width(context)*.05),
              child: Row(children: [
                MyText(text: "Height", size: StaticData.get_height(context)*.02),
                MyText(text:    widget.offer_product.shape==null? translator.translate("not found") :  "${widget.offer_product.shape} ${translator.translate("Cm")} ", size: StaticData.get_height(context)*.02),
  ...
  Widget product_review({List<offerModel.Rates> rates }){
    double width = MediaQuery.of(context).size.width;
    return rates.isEmpty? Container(
      height: width * 0.2,
      alignment: Alignment.center,
      child: Text(translator.translate("there is no comments")),
        ...
        itemBuilder: (context,index){
          ...
          var total_rate = (rates[index].deliveryTime+rates[index].product+rates[index].productQuality+rates[index].usingExperiences)/4;
          return Directionality(
            textDirection:  translator.currentLanguage == 'ar'? TextDirection.ltr : TextDirection.rtl,
            child: Column(
              children: [
                Container(
                  width: StaticData.get_width(context),
                  decoration: BoxDecoration(color:
                  whiteColor,borderRadius: BorderRadius.circular(
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              MyText(
                                  text: rates.isEmpty? '': rates[index].user==null?
                                  rates[index].user.name.cast<String,dynamic>() : translator.currentLanguage == 'ar'? "مستخدم" : "user",
                            ...
                            FittedBox(
                                ...
                                Container(
                                    ...
                                    child: MyText(
                                      ...
                                      size: StaticData.get_height(context)*.018,
                                      align:   TextAlign.end ,))),
                          ],),
                        Row(
                          children: [
                            Container(
                                padding: EdgeInsets.only(left: StaticData.get_width(context)*.05,right: StaticData.get_width(context)*.1),
                                child: MyText(text: rates[index].createDates.createdAtHuman,
                                  size: StaticData.get_height(context)*.02,
                                  color: greyColor,)),
