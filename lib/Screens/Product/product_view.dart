    ...
    switch (widget.department_name) {
      ...
      case 'relatedProducts':
        product_bloc.add(getRelatedProduct_click(
            product_id: widget.product_id, offset: offset));
        break;

      case 'ShowSecondLevelSubcCategoryProducts':
        product_bloc.add(getSecondLevelSubCategoryProducts(
            secon_level_subcategory_id: widget.category_id, offset: offset));
        break;
      case 'filter_result':
            ...
            builder: (context, state) {
              if (state is Done) {
                return StreamBuilder<List<product_model.Products>>(
                  stream:
                      product_bloc.second_level_subcatatgory_products_subject,
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      if (snapshot.data == null) {
                        return NoData(
      ...
      switch (widget.department_name) {
        ...
        case 'filter_result':
          product_bloc.add(FilterProductsEvent(
              ...
              size_id:
                  await sharedPreferenceManager.readInteger(CachingKey.SIZE_ID),
              offset: offset));
          break;
      }
    }
